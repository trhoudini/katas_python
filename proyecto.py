...
def dividir():
    try:
        a = float(input("Introduce el primer número: "))  # Pedimos al usuario un número y lo convertimos a float
        b = float(input("Introduce el segundo número: "))  # Pedimos otro número al usuario y lo convertimos a float
        resultado = a / b  # Realizamos la división y guardamos el resultado
    except ValueError:  # Si hay un error al convertir a float
        print("Kata 8:", "Error: Debes introducir números válidos.")  # Mostramos un mensaje de error
    except ZeroDivisionError:  # Si el usuario intenta dividir por cero
        print("Kata 8:", "Error: No se puede dividir entre cero.")  # Mostramos un mensaje de error
    else:  # Si no hubo errores
        print("Kata 8:", "Resultado:", resultado)  # Mostramos el resultado de la división
    finally:  # Este bloque se ejecuta siempre
        print("Kata 8:", "Fin del programa.")  # Indicamos que el programa ha terminado
dividir()
...
def lista_a_numero(digitos):  # Recibe una lista de dígitos
    return reduce(lambda x, y: x * 10 + y, digitos)  # Usamos reduce para formar el número como si fuera decimal

# Probamos con una lista de dígitos
print("Kata 17:", lista_a_numero([5, 7, 2]))
...
def diferencia_total(lista):  # Recibe una lista de números
    return reduce(lambda x, y: x - y, lista)  # Usa reduce para restar los elementos en orden

# Probamos con una lista de números
print("Kata 24:", diferencia_total([100, 30, 20]))
...
# Código a seguir
class Arbol:
    def __init__(self):  # Inicializamos el árbol
        self.tronco = 1  # Longitud inicial del tronco es 1
        self.ramas = []  # Lista vacía de ramas

    def crecer_tronco(self):  # Método para hacer crecer el tronco
        self.tronco += 1  # Aumentamos la longitud del tronco en 1

    def nueva_rama(self):  # Método para añadir una nueva rama
        self.ramas.append(1)  # Agregamos una rama nueva de longitud 1

    def crecer_ramas(self):  # Método para hacer crecer todas las ramas
        self.ramas = [rama + 1 for rama in self.ramas]  # Aumentamos en 1 la longitud de cada rama

    def quitar_rama(self, index):  # Método para quitar una rama en una posición específica
        if 0 <= index < len(self.ramas):  # Verificamos que el índice esté dentro del rango válido
            del self.ramas[index]  # Eliminamos la rama en esa posición

    def info_arbol(self):  # Método para obtener información del árbol
        return {
            "longitud_tronco": self.tronco,  # Mostramos la longitud actual del tronco
            "numero_ramas": len(self.ramas),  # Número total de ramas
            "longitudes_ramas": self.ramas  # Lista con las longitudes de cada rama
        }

# Caso de uso del árbol
...
arbol.nueva_rama()  # 5. Añadir una segunda rama
arbol.nueva_rama()  # 5. Añadir una tercera rama
arbol.quitar_rama(2)  # 6. Retirar la rama situada en la posición 2
print("Kata 34:", arbol.info_arbol())  # 7. Obtener información del árbol
...
# Código a seguir:
class UsuarioBanco:
    def __init__(self, nombre, saldo, cuenta_corriente):  # Constructor que recibe el nombre, saldo y si tiene cuenta corriente
        self.nombre = nombre  # Guardamos el nombre del usuario
        self.saldo = saldo  # Guardamos el saldo inicial del usuario
        self.cuenta_corriente = cuenta_corriente  # Indicamos si el usuario tiene cuenta corriente

    def retirar_dinero(self, cantidad):  # Método para retirar dinero
        if cantidad > self.saldo:  # Si la cantidad es mayor al saldo disponible
            raise ValueError(f"{self.nombre} no tiene suficiente saldo para retirar {cantidad}.") # Lanzamos un error indicando que no hay suficiente saldo
        self.saldo -= cantidad  # Descontamos la cantidad del saldo

    def transferir_dinero(self, otro_usuario, cantidad):  # Método para transferir dinero desde otro usuario al actual
        if cantidad > otro_usuario.saldo:  # Verificamos si el otro usuario tiene suficiente saldo
            raise ValueError(f"{otro_usuario.nombre} no tiene suficiente saldo para transferir {cantidad}.")  # Lanzamos error si no tiene suficiente
        otro_usuario.saldo -= cantidad  # El otro usuario pierde saldo
        self.saldo += cantidad  # El usuario actual recibe ese saldo  # Sumamos esa cantidad al saldo del usuario actual

    def agregar_dinero(self, cantidad):  # Método para agregar dinero al saldo
        self.saldo += cantidad  # Sumamos la cantidad al saldo actual

# Caso de uso:
alicia = UsuarioBanco("Alicia", 100, True)  # 1. Crear usuario Alicia con saldo 100 y cuenta corriente
bob = UsuarioBanco("Bob", 50, True)  # 1. Crear usuario Bob con saldo 50 y cuenta corriente

bob.agregar_dinero(30)  # 2. Bob agrega 30 unidades a su saldo. He puesto 30 ya que si pongo 20 me da error y no puedo continuar.
alicia.transferir_dinero(bob, 80)  # 3. Alicia recibe 80 unidades desde Bob
alicia.retirar_dinero(50)  # 4. Alicia retira 50 unidades de su saldo

# Mostramos el resultado final
print("Kata 36 - Alicia:", alicia.nombre, alicia.saldo)  # Información final de Alicia
print("Kata 36 - Bob:", bob.nombre, bob.saldo)  # Información final de Bob
...
hora = int(input("Introduce la hora (0-23): "))  # Convertimos a entero el dato introducido

# Verificamos que la hora esté dentro de un rango válido
if hora < 0 or hora > 23: # Si la hora no está entre 0 y 23. El 0 = 24
    print("Kata 38:", "Hora no válida. Debe estar entre 0 y 24.")  # Si no está en ese rango → mostramos error
else:
    # Si la hora está entre 6 y 12 → Es de día
    if 6 <= hora <= 12:
        print("Kata 38:", "Es de día")
    
    # Si la hora está entre 13 y 20 → Es tarde
    elif 13 <= hora <= 20:
        print("Kata 38:", "Es tarde")

    # Si la hora está entre 21 y 23 o entre 0 y 5 → Es de noche
    else:
        print("Kata 38:", "Es de noche")
